...
LLI GP23, #4 ; Allocate for 1 command struct + null, 2 pointers in the struct, and the command word itself (which includes null in our case)
CALLI :malloc ; Obtain the pointer, it's now on GP28

MOV GP10, GP28   ; Move heap pointer result into new reg, it's 4 words, we know.
...
; GP10 - Command struct pointers array
; GP11 - Pointer to add command struct
; GP12 - Pointer to command word itself
; GP22 - Main interpreter prompt
; Set prompt pointers
LLI GP0, .char_gt
SHLI GP0, #8
ADDI GP0, .char_space
...
ADDI GP1, #4     ; Command word pointer
ST GP0, GP1, #0  ; Store the word
MOV GP0, GP10
ADDI GP0, #2
ST GP1, GP0, #0  ; Store pointer to word
LLI GP0, :arithmetic_flow
ST GP0, GP1, #-1 ; Set function to call

; 2026-02-16 rework:
; GP10, #0 - Pointer to command struct
; GP10, #1 - Null
; GP10, #2 - Pointer to command word
; GP10, #3 - Pointer to function to call
; GP10, #4 - Command word (includes null, for add anyway. Some others may need multiple words)
; Old:
; GP10, #0 - Pointer to command struct
; GP10, #1 - Null
; GP10, #2 - Pointer to command word
...
LD GP1, GP0, #1 ; Load function pointer into GP1
MOV GP23, GP0
CALL GP1
JMP :main_loop
...
arithmetic_flow:
; GP23 - Command struct
; Stack? Print one word at a time?
; stack is annoying because it'll make us need to do this in reverse basically.
...
display_from_stack:
  ...
  do_loop:
    MOV GP2, GP0 ; Copy
    SHRI GP2, #24 ; Get top char
    CMP GP2, GP21 ; Check if null
    JEQ :end_of_text
    ST GP2, GP31, #1 ; Display on tty
    MOV GP2, GP0 ; Copy
    SHLI GP2, #8 ; Remove top char
    SHRI GP2, #24 ; remove bottom 2
    CMP GP2, GP21 ; Check if null
    JEQ :end_of_text
    ST GP2, GP31, #1
    MOV GP2, GP0 ; Copy
